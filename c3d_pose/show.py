...
filename='./output/work_test.txt'
#filename= './output/work.txt'
video_save = '/home/qbq/Desktop/c3d_pose/image_output/'
fps=20
...
'''
pose_dict= {'0': 'norm', '1':'phone','2':'smoking'}
...
def show_video(gt, path, inf):
    ...
    for i in range(len(inf)):
        ...
        for j in range(i*16, i*16 + 16):
            ...
            cv2.imshow('result', sMatImage)
            cv2.waitKey(50)
            
            ### save to image
            #new_ = frame.split('/')
            #b= new_[-2]
            #n_video_save=os.path.join(video_save, b)

            #if os.path.isdir(n_video_save) == False:
            #    os.mkdir(n_video_save)
            #save_path=os.path.join(n_video_save, frame_path)
            #cv2.imwrite(save_path, sMatImage)
